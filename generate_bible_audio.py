...
def clean_text(text: str) -> str:
    """Clean verse text before TTS: remove special chars and footnote markers."""
    # Remove cross-reference asterisks (*templo -> templo)
    text = text.replace('*', '')
    # Remove footnote brackets [a] [b] [1]
    text = re.sub(r'\[[a-z0-9]+\]', '', text)
    # Remove angle brackets
    text = re.sub(r'<[^>]+>', '', text)
    # Normalize Bible quotes
...
def generate_and_upload(book: str, chapter: int, verse: int, verse_text: str) -> str:
    ...
    try:
        # Step 1: Generate WAV with fixed seed for consistent voice across all verses
